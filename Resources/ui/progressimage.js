...
exports.create = function(_options) {
	var key, self = Ti.UI.createImageView({});
	for (key in _options) {
	...
	if(file.exists()) 
		...
		var client = Ti.Network.createHTTPClient({
			...
			onload : function() {
			  if (this.status === 200) {
			  	self.progress.hide(),file.write(this.responseData),self.setImage(file.nativePath);
			  };
			}
		});
		client.open("GET",_options.image),client.send(null);
	};	
